...
def connect_tip_cells(
  ...
  cluster_mat = compute_cluster_connectivities(
      matrix=matrix, clusters=clusters)
  cluster_mat_leaf = cluster_mat[leaves][:, leaves]
  # ignore connections of leaves to itself
  np.fill_diagonal(cluster_mat_leaf, qval_thresh + 1)
...
def calculate_projection(
  ...
  proj_factor = np.sum(
      (matrix - roots) * (leaves - roots), axis=1) / edge_length
...
def __learn_graph(
  ...
  if use_clusters_as_kmeans:
    ...
    if n_centroids is None:
      ...
      n_centroids = int(n_cluster * centroids_per_log *
                        np.log10(matrix.shape[0]))
    n_centroids = max(n_centroids, MINIMUN_CENTROIDS)
    n_centroids = min(n_centroids, matrix.shape[0])

    if n_centroids < matrix.shape[0]:
      # Init centroids
      np.random.seed(kmeans_seed)
      init = matrix[np.linspace(0, matrix.shape[0] - 1,
                                num=n_centroids, dtype=int)]
